...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void optionalAttendeesUnavailable() {
    // Have each person have different events. We should see three options because each person has
    // split the restricted times, and no time slots exists that both mandatory and optional
    // attendees can attend. (based on everyAttendeeIsConsidered)
    //
    // Events  : |--------------C--------------|     <- only for optional attendee
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  public void optionalAttendeesAvailable() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times, and 2 time slots exist that both mandatory and optional
    // attendees can attend. (based on everyAttendeeIsConsidered)
    //
    // Events  :             |--C--|                <- only for optional attendee
    // Events  :       |--A--|     |--B--|
  ...
  public void optionalAttendeesIgnored() {
    // There is just enough room at one point in the day to have the meeting, and the
    // optional attendee is ignored. (based on justEnoughRoom)
    //
    // Events  :       |-B-|                 <- only for optional attendee
    // Events  : |--A--|     |----A----|
    // Day     : |---------------------|
    // Options :       |-----|
  ...
  @Test
  public void optionalAttendeesOnlyAndAreAvailable() {
    ...
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected = 
        Arrays.asList(TimeRange.fromStartDuration(TIME_0800AM, DURATION_1_HOUR));

    Assert.assertEquals(expected, actual);
  }

  @Test
  public void optionalAttendeesOnlyAndAreNotAvailable() {
